...
const ENDPOINTS = {
  ...
  vpnPublicNetworkStats: `${API_BASE_URL}/privacy_network/vpn/public/network-stats/`
};
...
/**
 * Fetch public commercial VPN aggregate statistics.
 * Returns privacy-safe totals only; no node keys, client IPs, destinations,
 * DNS contents, packet payloads, wallet-level traffic, or browsing history.
 */
export const fetchVpnNetworkStats = async () => {
  try {
    const response = await fetch(ENDPOINTS.vpnPublicNetworkStats);

    if (!response.ok) {
      const errorText = await response.text().catch(() => 'Unknown error');
      console.error(`Failed to fetch VPN network stats: ${response.status} - ${errorText}`);
      throw new Error(`Failed to fetch VPN network stats: ${response.status}`);
    }

    return await response.json();
  } catch (error) {
    console.error('Error fetching VPN network stats:', error);
    return null;
  }
};
...
export const fetchAllNetworkStats = async (period = '30d') => {
  try {
    ...
    let nodesData = null, faucetData = null, synxData = null, vpnData = null;
    ...
    try {
      vpnData = await fetchVpnNetworkStats();
    } catch (e) {
      console.error('Error in VPN network API fetch:', e);
    }
    
    console.log('All data fetched:', { nodesData, faucetData, synxData, vpnData });
    ...
    const vpnPayload = vpnData?.data || {};
    const vpnNetwork = vpnPayload.network || {};
    const vpnSessions = vpnPayload.sessions || {};
    const vpnTraffic = vpnPayload.encrypted_traffic || {};
    const vpnMessages = vpnPayload.encrypted_message_forwarding || {};
    ...
    return {
      ...
      metrics: {
        ...
        vpnOnlineNodes: formatNumber(vpnNetwork.online_vpn_nodes || 0),
        vpnTotalNodes: formatNumber(vpnNetwork.vpn_nodes || 0),
        vpnActiveSessions: formatNumber(vpnSessions.active_sessions || 0),
...
export const formatDataVolume = (bytes) => {
  const value = parseNumericValue(bytes);
  if (value >= 1024 ** 4) {
