...
/**
 * Fetch public AeroNyx privacy protocol aggregate statistics.
...
export const fetchVpnNetworkStats = async () => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => 'Unknown error');
      console.error(`Failed to fetch privacy protocol stats: ${response.status} - ${errorText}`);
...
export const fetchAllNetworkStats = async (period = '30d') => {
  try {
    ...
    try {
      ...
      console.error('Error in privacy protocol API fetch:', e);
    }
