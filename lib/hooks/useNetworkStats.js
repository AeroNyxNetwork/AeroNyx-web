 ...
 *
 * Last Modified: v1.5.1 - Parse coordinator witness-certificate coverage and
 * pending block counts separately from the network-wide maximum certificate
 * height. This prevents a follower's aggregate value from being presented as
 * the production coordinator's current block confirmation state.
 * Previous: v1.5.0 - Parse the backend-owned signed commitment ledger
 * aggregate. Public UI receives only counts and witness-lease health; hashes,
 * signatures, witness identities, endpoints, owners, and memory contents stay
 * outside the website contract.
 * Previous: v1.4.3 - Added Rust discovery summary source fields from
 * protocol_status.discovery_summary_reported_nodes and
 * protocol_status.protocol_foundation.two_hop_path_proof_history.source so
 * the homepage can explain protocol health as product-level proof instead of
 * exposing raw engineering counters.
 * Previous: v1.4.2 - Added two-hop path proof history parsing from
 * protocol_status.protocol_foundation.two_hop_path_proof_history so the
 * homepage can show aggregate multi-hop readiness without exposing node IDs,
 * endpoints, route IDs, encrypted payloads, receiver identities, client IPs,
 * DNS contents, Memory Chain plaintext, wallet-level traffic, or social graph
 * edges.
 * Previous: v1.4.1 - Added protocol foundation relay readiness reason parsing
  ...
  const processNetworkData = (rawData) => {
    if (!rawData) {
      return {
        ...
        protocolStatusSource: 'node_hardware_info_discovery_status',
        protocolDiscoverySummaryReportedNodes: 0,
        ...
        protocolFoundationRelayEvidenceMode: 'idle',
        protocolFoundationRelayReadinessReason: 'idle_waiting_for_relay',
        protocolFoundationRealRelayReadyNodes: 0,
        protocolFoundationSyntheticProbeReadyNodes: 0,
        protocolFoundationTwoHopProofReadyNodes: 0,
        protocolFoundationTwoHopProofReportedNodes: 0,
        protocolFoundationTwoHopProofRetainedEvents: 0,
        protocolFoundationTwoHopProofSuccessPercent: 0,
        protocolFoundationTwoHopProofFreshness: 'syncing',
        protocolFoundationTwoHopProofSource: null,
        protocolFoundationTwoHopProofLatestSuccessAgeSeconds: null,
        protocolFoundationEvidenceModeCounts: {},
        protocolFoundationReadinessReasonCounts: {},
        ...
        protocolMemoryChainMode: 'signed_commitment_ledger',
        protocolMemoryChainStatus: 'syncing',
        protocolMemoryChainConsensus: 'not_claimed',
        protocolBlockConfirmationState: 'unavailable',
        protocolCommitmentReportedNodes: 0,
        protocolCommitmentVerifiedNodes: 0,
        protocolCommitmentCoordinatorNodes: 0,
        protocolCommitmentFollowerNodes: 0,
        protocolCommitmentVerifiedBlocks: 0,
        protocolCommitmentVerifiedCommitments: 0,
        protocolCommitmentTipHeight: 0,
        protocolCommitmentCertifiedTipHeight: 0,
        protocolCommitmentUncertifiedBlocks: 0,
        protocolCommitmentLeaseGrantedWitnesses: 0,
        protocolCommitmentLeaseRequiredWitnesses: 0,
        protocolCommitmentLeaseHeldCoordinators: 0,
        protocolCommitmentProductionHaltedNodes: 0,
    ...
    const protocolTwoHopProof = protocolFoundation.two_hop_path_proof_history || {};
    const protocolTwoHopProofFreshnessCounts = (
      protocolTwoHopProof.freshness_counts
      && typeof protocolTwoHopProof.freshness_counts === 'object'
      && !Array.isArray(protocolTwoHopProof.freshness_counts)
        ? protocolTwoHopProof.freshness_counts
        : {}
    );
    const protocolTwoHopProofFreshness = Object.entries(protocolTwoHopProofFreshnessCounts)
    ...
    const additionalMetrics = {
      ...
      protocolStatusSource: protocolStatus.source || 'node_hardware_info_discovery_status',
      protocolDiscoverySummaryReportedNodes: Number(protocolStatus.discovery_summary_reported_nodes || 0),
      ...
      protocolFoundationRouteableRelayCount: Number(
        protocolFoundation.routeable_relay_count
        ?? protocolFoundation.max_routeable_relay_count
        ?? protocolPeerQuorum.max_routeable_chat_relays
        ?? protocolNetworkStory.max_routeable_chat_relays
        ?? 0
      ),
      protocolFoundationLastProbeAgeSeconds: (
        typeof protocolFoundation.last_probe_age_seconds === 'number'
          ...
          : typeof protocolFoundation.min_last_probe_age_seconds === 'number'
            ? protocolFoundation.min_last_probe_age_seconds
            : null
      ),
      protocolFoundationRelayEvidenceMode: (
        protocolFoundation.relay_evidence_mode
        || protocolFoundation.evidence_mode
        || (Number(protocolFoundation.real_relay_ready_nodes || 0) > 0
          ? 'real_relay_traffic'
          : Number(protocolFoundation.synthetic_probe_ready_nodes || 0) > 0
            ? 'synthetic_probe'
            : 'idle')
      ),
      protocolFoundationRelayReadinessReason: (
        protocolFoundation.relay_readiness_reason
        || protocolFoundation.readiness_reason
        || (Number(protocolFoundation.real_relay_ready_nodes || 0) > 0
          ? 'real_relay_observed'
          : Number(protocolFoundation.synthetic_probe_ready_nodes || 0) > 0
      ...
      protocolFoundationRealRelayReadyNodes: Number(
        protocolFoundation.real_relay_ready_nodes
        ?? (protocolFoundation.real_relay_ready === true ? 1 : 0)
        ?? 0
      ),
      protocolFoundationSyntheticProbeReadyNodes: Number(
        protocolFoundation.synthetic_probe_ready_nodes
        ?? (protocolFoundation.synthetic_probe_ready === true ? 1 : 0)
        ?? 0
      ),
      protocolFoundationTwoHopProofReadyNodes: Number(
        protocolFoundation.two_hop_path_proof_ready_nodes
        ?? protocolTwoHopProof.proof_ready_nodes
        ?? 0
      ),
      protocolFoundationTwoHopProofReportedNodes: Number(
        protocolTwoHopProof.reported_nodes
        ?? protocolFoundation.reported_nodes
        ?? protocolStatus.reported_nodes
        ?? 0
      ),
      protocolFoundationTwoHopProofRetainedEvents: Number(
        protocolTwoHopProof.retained_events ?? 0
      ),
      protocolFoundationTwoHopProofSuccessPercent: Number(
        protocolTwoHopProof.success_percent ?? 0
      ),
      protocolFoundationTwoHopProofFreshness: (
        protocolTwoHopProofFreshness || 'syncing'
      ),
      protocolFoundationTwoHopProofSource: protocolTwoHopProof.source || null,
      protocolFoundationTwoHopProofLatestSuccessAgeSeconds: (
        typeof protocolTwoHopProof.min_latest_success_age_seconds === 'number'
          ? protocolTwoHopProof.min_latest_success_age_seconds
          : null
      ),
      protocolFoundationEvidenceModeCounts: (
        protocolFoundation.evidence_mode_counts
        && typeof protocolFoundation.evidence_mode_counts === 'object'
        && !Array.isArray(protocolFoundation.evidence_mode_counts)
          ? protocolFoundation.evidence_mode_counts
          : {}
      ),
      protocolFoundationReadinessReasonCounts: (
        protocolFoundation.readiness_reason_counts
        && typeof protocolFoundation.readiness_reason_counts === 'object'
        && !Array.isArray(protocolFoundation.readiness_reason_counts)
          ? protocolFoundation.readiness_reason_counts
          : {}
      ),
      ...
      protocolMemoryChainMode: protocolMemoryChain.mode || 'signed_commitment_ledger',
      protocolMemoryChainStatus: protocolMemoryChain.status || 'syncing',
      protocolMemoryChainConsensus: protocolMemoryChain.network_consensus || 'not_claimed',
      protocolBlockConfirmationState: protocolMemoryChain.block_confirmation_state || 'unavailable',
      protocolCommitmentReportedNodes: Number(protocolMemoryChain.reported_nodes || 0),
      protocolCommitmentVerifiedNodes: Number(protocolMemoryChain.verified_nodes || 0),
      protocolCommitmentCoordinatorNodes: Number(protocolMemoryChain.coordinator_nodes || 0),
      protocolCommitmentFollowerNodes: Number(protocolMemoryChain.follower_nodes || 0),
      protocolCommitmentVerifiedBlocks: Number(protocolMemoryChain.max_verified_block_count || 0),
      protocolCommitmentVerifiedCommitments: Number(protocolMemoryChain.max_verified_commitment_count || 0),
      protocolCommitmentTipHeight: Number(protocolMemoryChain.max_verified_tip_height || 0),
      protocolCommitmentCertifiedTipHeight: Number(
        protocolMemoryChain.coordinator_certified_tip_height
        ?? protocolMemoryChain.max_certified_tip_height
        ?? 0
      ),
      protocolCommitmentUncertifiedBlocks: Number(
        protocolMemoryChain.coordinator_uncertified_block_count
        ?? protocolMemoryChain.max_uncertified_block_count
        ?? 0
      ),
      protocolCommitmentLeaseGrantedWitnesses: Number(protocolMemoryChain.max_granted_witnesses || 0),
      protocolCommitmentLeaseRequiredWitnesses: Number(protocolMemoryChain.max_required_witnesses || 0),
      protocolCommitmentLeaseHeldCoordinators: Number(protocolMemoryChain.lease_held_coordinator_nodes || 0),
      protocolCommitmentProductionHaltedNodes: Number(protocolMemoryChain.production_halted_nodes || 0),
